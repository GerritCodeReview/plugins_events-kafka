...
public class KafkaEventRestSubscriber implements KafkaEventSubscriber {
  ...
  private void runReceiver(String topicName) throws InterruptedException, ExecutionException {
    receiver = new ReceiverJob(topicName);
  ...
  private class ReceiverJob implements Runnable {
    ...
    public ReceiverJob(String jobName) throws InterruptedException, ExecutionException {
      closed = new AtomicBoolean();
      consumerName = String.format("%s_%d", jobName, hashCode());
      kafkaRestConsumerUri = createConsumer(consumerName);
      kafkaSubscriber =
          Futures.transformAsync(kafkaRestConsumerUri, this::subscribeToTopic, futureExecutor);
    }

    public void close() {
      Futures.transformAsync(kafkaRestConsumerUri, this::deleteConsumer, futureExecutor);
      closed.set(true);
    }
    ...
    private void consume() throws InterruptedException, ExecutionException {
      try {
        ...
        while (!closed.get()) {
          ...
          consumerRecords
              .get(configuration.getPollingInterval().longValue(), TimeUnit.MILLISECONDS)
              .forEach(
                  consumerRecord -> {
    ...
    private ListenableFuture<URI> createConsumer(String name)
      ...
      post.setEntity(
          new StringEntity(
              String.format(
                  "{\"name\": \"%s\", \"format\": \"json\", \"auto.offset.reset\": \"earliest\"}",
                  name),
              ContentType.create(KAFKA_V2, StandardCharsets.UTF_8)));
      return Futures.transformAsync(
          JdkFutureAdapters.listenInPoolThread(httpclient.execute(post, null), futureExecutor),
      ...
      switch (response.getStatusLine().getStatusCode()) {
        case HttpStatus.SC_OK:
          try (Reader bodyReader = new InputStreamReader(response.getEntity().getContent())) {
            ...
            Stream<JsonObject> jsonObjects =
                StreamSupport.stream(jsonRecords.spliterator(), false)
                    .map(JsonElement::getAsJsonObject);

            jsonObjects.collect(Collectors.toMap(this::jsonRecordPartition, this::jsonRecordValue));
    ...
    private TopicPartition jsonRecordPartition(JsonObject jsonRecord) {
      return new TopicPartition(topic, jsonRecord.get("partition").getAsInt());
    }

    private Object jsonRecordValue(JsonObject jsonRecord) {
      return null;
    }
