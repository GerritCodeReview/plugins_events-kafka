...
public class KafkaRestProducer implements Producer<String, String> {
  private static final String KAFKA_V2_JSON = "application/vnd.kafka.v2+json";
  ...
  public Future<RecordMetadata> send(ProducerRecord<String, String> record, Callback callback) {
    try (CloseableHttpAsyncClient httpclient = HttpAsyncClients.createDefault()) {
      ...
      ListenableFuture<HttpResponse> responseListenableFuture =
          JdkFutureAdapters.listenInPoolThread(httpclient.execute(post, null), futureExecutor);
      return Futures.transformAsync(
          responseListenableFuture, this::getRecordMetadataResult, futureExecutor);
    } catch (IOException e) {
