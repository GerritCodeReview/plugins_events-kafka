...
public interface KafkaEventSubscriber {
  ...
  /**
   * Determines whether the consumer's group id is set to the value read from the configuration or
   * if it has been explicitly set.
   *
   * @return {@code true} if the consumer's group id set from configuration, {@code false} if set
   *     explicitly.
   */
  boolean isConfiguredGroupId();
}
