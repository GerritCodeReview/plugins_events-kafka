...
public class Manager implements LifecycleListener {
  ...
  @Inject
  public Manager(KafkaPublisher publisher, Set<TopicSubscriber> consumers, Set<TopicSubscriberWithGroupId> consumersWithGroupId, BrokerApi brokerApi) {
  ...
  public void start() {
    ...
    consumersWithGroupId.forEach(
            topicSubscriber ->
                    brokerApi.receiveAsync(topicSubscriber.topicSubscriber().topic(), topicSubscriber.groupId(), topicSubscriber.topicSubscriber().consumer()));
  }
